...
class TestOnboarding:
    ...
    def test_complete_onboarding(self, driver):  
        ...
        # toast_amigo = amigo_page.get_toast_message(timeout=15)
        # amigo_es_verde = amigo_page.verificar_color_snackbar(AmigoLocators.TOAST, 12, 194, 99,"amigo_snackbar.png")  # Verifica que el snackbar tenga el color verde esperado
        # amigo_page.verificar_toast_message(toast_amigo, "Código verificado correctamente")
        # assert amigo_es_verde, "El snackbar no es del color verde esperado"

        # dashboard_page.wait_text_visible(timeout=20)
        # dashboard_page.click_Menu_button()

        # menu_page.click_Perfil_button()
        # menu_page.click_Eliminar_cuenta_button()
        # menu_page.introduccir_confirmacion_eliminar_cuenta("ADIÓS TRIBBU")
        # menu_page.confirmar_eliminar_cuenta()

        # verificar Mayusculas y minúsculas en la confirmación de eliminación de cuenta

        # assert onboarding_page.wait_text_visible(timeout=15) == True, "No se volvió a la pantalla de onboarding después de eliminar la cuenta"
